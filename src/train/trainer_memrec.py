...
class MemRecTrainer:
    ...
    def __init__(
        ...
        # FeedbackMemRec V1 configuration. Text memory and graph topology stay unchanged.
        self.feedback_credit_config = config.get('feedback_credit', {})
        self.read_credit_config = config.get('read_credit', {})
        self.write_gate_config = config.get('write_gate', {})
        self.attribution_config = config.get('attribution', {})
        self.neighbor_memory_read_config = config.get(
            'neighbor_memory_read', {}
        )
        self.persist_memory_after_warmup = config.get(
            'persist_memory_after_warmup',
            self.neighbor_memory_read_config.get('enabled', False),
        )
        self.feedback_credit_enabled = self.feedback_credit_config.get('enabled', False)
        self.learn_credit = (
            self.feedback_credit_enabled
            and self.feedback_credit_config.get('learn_credit', False)
            and self.attribution_config.get('enabled', False)
        )
        self.freeze_credit = self.feedback_credit_config.get('freeze_credit', True)
        self.feedback_step = 0
        self.credit_updates_after_warmup = 0
        self.run_dir = None
        ...
        if self.reranker_mode == "llm":
            ...
            reranker_llm_model = reranker_provider_config.get(
                'model', self.llm_model
            )
            reranker_provider_name = reranker_provider_config.get(
                'name', provider_name
            )
            reranker_endpoint = reranker_provider_config.get(
                'endpoint', self.api_endpoint
            )
            reranker_api_key = reranker_provider_config.get(
                'api_key', self.api_key
            )
        ...
        self.neighbor_memory_tokenizer = None
        if self.neighbor_memory_read_config.get('enabled', False):
            if self.neighbor_memory_read_config.get('truncation', 'head_tail') != 'head_tail':
                raise ValueError(
                    "V1.2 only supports neighbor_memory_read.truncation=head_tail"
                )
            from transformers import AutoTokenizer

            tokenizer_path = self.neighbor_memory_read_config.get(
                'tokenizer_path', f'models/{self.llm_model}'
            )
            tokenizer_path = Path(tokenizer_path)
            if not tokenizer_path.is_absolute():
                tokenizer_path = Path(__file__).parent.parent.parent / tokenizer_path
            print(f"  Loading V1.2 tokenizer from {tokenizer_path}")
            self.neighbor_memory_tokenizer = AutoTokenizer.from_pretrained(
                str(tokenizer_path),
        ...
        self.agent = MemRecAgent(
            ...
            feedback_credit_config=self.feedback_credit_config,
            read_credit_config=self.read_credit_config,
            write_gate_config=self.write_gate_config,
            attribution_config=self.attribution_config,
            neighbor_memory_read_config=self.neighbor_memory_read_config,
            tokenizer=self.neighbor_memory_tokenizer,
            debug=self.debug
        )
        ...
        print(
            "  FeedbackMemRec: "
            f"credit={self.feedback_credit_enabled}, "
            f"learn={self.learn_credit}, "
            f"read={self.read_credit_config.get('enabled', False)}, "
            f"write_gate={self.write_gate_config.get('enabled', False)}, "
            f"freeze_test={self.freeze_credit}"
        )
        print(
            "  Neighbor memory read: "
            f"enabled={self.neighbor_memory_read_config.get('enabled', False)}, "
            f"user_max={self.neighbor_memory_read_config.get('max_user_neighbors', 4)}, "
            f"item_max={self.neighbor_memory_read_config.get('max_item_neighbors', 4)}"
        )
    ...
    def _log_candidate_manifest(
        self, phase: str, user_id: int, target_item: int, candidates: List[int]
    ) -> None:
        ...
        event = {
            ...
            'candidates': [int(item_id) for item_id in candidates],
        }
        path = Path(self.run_dir) / 'candidate_manifest.jsonl'
        with path.open('a', encoding='utf-8') as handle:
            handle.write(json.dumps(event, ensure_ascii=False) + '\n')

    def _append_observation(self, filename: str, event: Dict) -> None:
        """Append a result-only observation without changing model state."""
        if self.run_dir is None:
            return
    ...
    def _log_per_user_metric(
        ...
        """Record metrics derived from an already completed test ranking."""
        rank = int(target_rank)
        event = {
            'block': self.config.get('evaluation_block'),
            ...
            'target_item': int(target_item),
            'target_rank': rank,
            'ranking_success': bool(ranking_success),
            'hit_at_1': int(rank < 1),
            'hit_at_3': int(rank < 3),
            'hit_at_5': int(rank < 5),
            'hit_at_10': int(rank < 10),
            'ndcg_at_10': float(1.0 / np.log2(rank + 2)) if rank < 10 else 0.0,
        }
        if error is not None:
            event['error'] = error
        self._append_observation('per_user_metrics.jsonl', event)

    def _log_warmup_event(self, event: Dict) -> None:
        """Record warmup outcome only; never feed it back into the algorithm."""
        observation = {
            'block': self.config.get('evaluation_block'),
            'seed': int(self.config.get('seed', 42)),
            **event,
        }
        self._append_observation('warmup_events.jsonl', observation)
    ...
    def evaluate(self, split: str = 'test', save_dir: str = None, parallel: bool = False, n_workers: int = 16) -> Dict:
        ...
        if parallel and (
            self.feedback_credit_enabled
            or self.read_credit_config.get('enabled', False)
            or self.write_gate_config.get('enabled', False)
            or self.neighbor_memory_read_config.get('enabled', False)
        ):
            raise ValueError(
                "FeedbackMemRec stateful experiments must run serially; disable --parallel"
            )
        ...
        if (
            self.persist_memory_after_warmup
            and self.run_dir is not None
        ):
            warmup_memory_path = Path(self.run_dir) / 'memory_after_warmup.jsonl'
            self.agent.storage.save_to_jsonl(
                str(warmup_memory_path), dataset=self.dataset
            )
        ...
        # ========== Serial Mode (original logic, debug support retained) ==========
            ...
            for user_id in tqdm(eval_user_ids, desc=f"MemRec {split}"):
                ...
                self._log_candidate_manifest(split, user_id, target_item, candidates)
                ...
                try:
                    ...
                    ranked_items, details = self.agent.rerank(
                        ...
                        episode_id=f"test-u{user_id}-t{target_item}",
                        phase='test',
                    )
                    ...
                    self._log_per_user_metric(
                        split=split,
                        user_id=user_id,
                        target_item=target_item,
                        target_rank=pos,
                        ranking_success=(target_item in ranked_items),
                    )
                    
                    # Stage-W: Write (based on eval_feedback mode)
                    try:
                        attribution_result = None
                        write_episode_id = f"test-{split}-u{user_id}"
                        if self.learn_credit and not self.freeze_credit:
                            write_episode_id = (
                                f"prequential-{split}-u{user_id}-s{self.feedback_step}"
                            )
                            attribution_result = self.agent.attribute_facets(
                                episode_id=write_episode_id,
                        ...
                        if self.eval_feedback != 'none':
                            ...
                            if feedback:
                                ...
                                write_result = self.agent.write(
                                    ...
                                    pruned_subgraph=details.get('pruned_subgraph', None),
                                    episode_id=write_episode_id,
                                    current_relation_deltas=(
                                        attribution_result.get('raw_relation_deltas', [])
                                        if attribution_result else []
                                    ),
                                    debug_logger=self._log_debug if self.debug else None
                                )
                                stage_w_time = time.time() - t0
                    ...
                    print(f"\nError evaluating user {user_id}: {e}")
                    failed_rank = len(candidates)
                    ranking_positions.append(failed_rank)
                    self._log_per_user_metric(
                        split=split,
                        user_id=user_id,
                        target_item=target_item,
        ...
        metrics['evidence_credit_stats'] = storage_stats['evidence_credit']
        metrics['feedback_attribution_stats'] = storage_stats['feedback_attribution']
        metrics['neighbor_memory_read_stats'] = storage_stats[
            'neighbor_memory_read'
        ]
        metrics['credit_updates_during_test'] = (
            storage_stats['evidence_credit']['total_updates']
            - self.credit_updates_after_warmup
        )
        print(f"\nStorage statistics:")
    ...
    def _warmup_single_user(self, user_id, warmup_round, target_offset, split, lock=None):
        ...
        self._log_candidate_manifest(
            f'warmup-{warmup_round}', user_id, target_item, candidates
        )
        ...
        try:
            ...
            ranked_items, details = self.agent.rerank(
                ...
                episode_id=f"warmup-r{warmup_round}-u{user_id}-t{target_item}",
                phase='warmup',
            )
            ...
            if target_pos < 10:  # top-10 triggers Stage-W
                ...
                write_result = self.agent.write(
                    ...
                    recent_facets=details.get('facets', []),
                    pruned_subgraph=details.get('pruned_subgraph', None),
                    episode_id=f"warmup-r{warmup_round}-u{user_id}",
                    debug_logger=None  # Parallel mode doesn't support debug
                )
    ...
    def _run_training(
                ...
                for user_id in tqdm(eval_user_ids, desc=f"MemRec Training"):
                    ...
                    try:
                        ranked_items, details = self.agent.rerank(
                            ...
                            episode_id=(
                                f"warmup-r{warmup_round}-u{user_id}-t{target_item}"
                            ),
                            phase='warmup',
                        )
                        ...
                        if target_pos < 10:  # top-10 triggers Stage-W (relaxed condition to increase training success rate)
                            ...
                            # Strict order: predict -> reveal/evaluate -> attribute/update credit -> write.
                            attribution_result = None
                            if self.learn_credit:
                                attribution_result = self.agent.attribute_facets(
                                ...
                                if self.debug:
                                    self._log_debug(
                                        "Attribution: "
                                        f"{attribution_result.get('n_facets', 0)} facets, "
                                        f"{attribution_result.get('n_relation_updates', 0)} relations"
                                    )
                            ...
                            self._log_warmup_event({
                                ...
                                'user_id': int(user_id),
                                'target_item': int(target_item),
                                'target_rank': int(target_pos),
                                'status': 'updated',
                                'ranking_success': True,
                                'memory_updated': True,
                                'patches_applied': int(n_patches),
                            })
                        else:
                            self._log_warmup_event({
                                'phase': f'warmup-{warmup_round}',
                                ...
                                'target_item': int(target_item),
                                'target_rank': int(target_pos),
                                'status': 'rank_outside_write_cutoff',
                                'ranking_success': True,
                                'memory_updated': False,
                            })
                            if self.debug:
        ...
        self.credit_updates_after_warmup = self.agent.credit_store.get_stats()[
            'total_updates'
        ]
    ...
    def train(self, save_dir: str = None) -> Dict:
        ...
        if save_dir:
            feedback_log_dir = Path(save_dir) / "feedback_memrec_logs"
            self.agent.configure_feedback_output(feedback_log_dir)
    ...
    def test(self, parallel: bool = False, n_workers: int = 16) -> Dict:
        ...
        if save_dir:
            ...
            credit_path = Path(save_dir) / "evidence_credit.json"
            self.agent.save_credit_store(credit_path)
