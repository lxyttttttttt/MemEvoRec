...
class NeighborPruner:
    ...
    def _apply_read_credit(
        self, user_id: int, neighbor_type: str, neighbor_id: int, score: float
    ) -> Tuple[float, float, float]:
        if not self.read_credit_config.get('enabled', False) or self.credit_store is None:
            return float(score), 0.0, 1.0
        evidence_type = f"{neighbor_type}_neighbor"
        key = make_relation_key(user_id, evidence_type, neighbor_id)
    ...
    def prune(
        ...
        for item_id, score in scored['item_scores']:
            original_score = score * self.item_weight
            weighted_score, credit_q, credit_multiplier = self._apply_read_credit(
                user_id, 'item', item_id, original_score
            )
        ...
        for neighbor_user_id, score in scored['user_scores']:
            original_score = score * self.user_weight
            weighted_score, credit_q, credit_multiplier = self._apply_read_credit(
                user_id, 'user', neighbor_user_id, original_score
            )
            all_neighbors.append({
                'type': 'user',
                'id': neighbor_user_id,
