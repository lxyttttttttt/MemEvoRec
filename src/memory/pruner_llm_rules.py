...
from .evidence_credit import make_relation_key
...
class LLMRulePruner:
    ...
    def __init__(
        ...
        mix_min_items: int = 6,
        credit_store=None,
        read_credit_config: Optional[Dict] = None,
    ):
        """
        ...
        self.read_credit_config = read_credit_config or {}
    ...
    def _apply_read_credit(self, user_id: int, neighbor: Dict, score: float):
        if not self.read_credit_config.get('enabled', False) or self.credit_store is None:
            return float(score), 0.0, 1.0
        key = make_relation_key(
            user_id,
            f"{neighbor['type']}_neighbor",
            neighbor['id'],
        )
        q = self.credit_store.get_q(key)
        multiplier = self.credit_store.get_multiplier(
            key,
            lambda_credit=self.read_credit_config.get('lambda_credit', 0.5),
            min_multiplier=self.read_credit_config.get('min_multiplier', 0.5),
            max_multiplier=self.read_credit_config.get('max_multiplier', 1.5),
        )
        return float(score * multiplier), float(q), float(multiplier)
    ...
    def prune(
        ...
        for neighbor in candidate_neighbors:
            original_selection_score = self.score_neighbor(neighbor, user_memory, None)
            selection_score, credit_q, credit_multiplier = self._apply_read_credit(
                user_id, neighbor, original_selection_score
            )
            neighbor = dict(neighbor)
            neighbor['original_selection_score'] = float(original_selection_score)
            neighbor['selection_score'] = float(selection_score)
            neighbor['credit_q'] = credit_q
            neighbor['credit_multiplier'] = credit_multiplier
            scored_neighbors.append((selection_score, neighbor))
