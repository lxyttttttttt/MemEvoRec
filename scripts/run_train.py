...
def main():
    ...
    with (save_dir / "resolved_config.json").open("w", encoding="utf-8") as f:
    ...
    test_metrics['wall_time_seconds'] = time.time() - run_started

    # Keep a self-contained metrics artifact alongside the run's state/log files.
    run_metrics = {
        'config': config,
        'valid_metrics': best_state.get('metrics', {}),
        'test_metrics': test_metrics,
    }
    with (save_dir / 'run_metrics.json').open('w', encoding='utf-8') as f:
